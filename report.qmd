...
For the data analysis, we intend to utilize visualizations of the regional and global homicide statistics to derive our findings as related to our first question. For the second question, we will be using a visualization with gender breakdown **(DRAFT note: can do regionally if needed)**. For the third question, we will be utilizing a series of regression models to determine the level of correlation between homicides and GDP via their R\^2 values.
...
homicides_and_crime_un_sex <- homicides_and_crime_un |>
  filter(Region.Country.Area == "1") |>
  mutate(sex = if_else(str_detect(Series, "Male"), "Male", "Female")) |>
  filter(Series != "Intentional homicide rates per 100,000")
...
**RQ1 - Global and Regional Statistics**
...
Broken down by sex, it seems that **men are much more likely to be victims of homicide than women**. This is unsurprising, not only due to the known likelihood of men being involved in violent/aggressive activity but especially considering that the global population breakdown is 101.68 men per 100 women as of 2020.

**RQ3 - Regressions - Regional and Global**
